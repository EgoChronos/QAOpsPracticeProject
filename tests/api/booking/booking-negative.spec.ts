...
test.describe('Booking API — Negative Testing', () => {
  ...
  test.describe('Non-existent bookings', () => {
    test('should return 404 when getting a booking with non-existent ID @regression @api @negative', async ({
      bookingClient,
    }) => {
      try {
    ...
    test('should return 405 or error when deleting non-existent booking without auth @regression @api @negative', async () => {
  ...
  test.describe('Unauthenticated write operations', () => {
    test('should return 403 when updating without auth token @regression @api @negative', async ({
      bookingService,
    }) => {
    ...
    test('should return 403 when deleting without auth token @regression @api @negative', async ({
      bookingService,
    }) => {
      const created = await bookingService.createBooking(BookingFactory.create());
  ...
  test.describe('Data validation', () => {
    test('should handle booking with minimum required fields @regression @api @negative', async ({
      bookingService,
    }) => {
    ...
    test('should retrieve booking without additionalneeds field @regression @api @negative', async ({
      bookingService,
    }) => {
      const payload = {
        firstname: 'NoNeeds',
  ...
  test.describe('Filter edge cases', () => {
    test('should return empty array for non-existent firstname filter @regression @api @negative', async ({
      bookingClient,
    }) => {
