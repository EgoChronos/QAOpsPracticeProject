...
test.describe('Auth API', () => {
  ...
  test.describe('POST /auth', () => {
    test('should return a token with valid admin credentials @smoke @api @auth', async ({
      authClient,
    }) => {
    ...
    test('should return 200 status code even for invalid credentials @regression @api @auth', async ({
      authClient,
    }) => {
    ...
    test('should return bad credentials reason for invalid credentials @regression @api @auth', async ({
      authClient,
    }) => {
      const response = await authClient.createToken({
        username: 'wrong_user',
